...
describe("Device Status", () => {
    ...
    it("allows polling device reachability", async () => {
        fetchMock.mockGlobal().post(
            "https://network-as-code.p-eu.rapidapi.com/device-reachability-status/v1/retrieve",
            (_: any, req: any): any => {
    ...
    it("allows polling device roaming status", async () => {
        fetchMock.mockGlobal().post(
            "https://network-as-code.p-eu.rapidapi.com/device-roaming-status/v1/retrieve",
            (_: any, req: any): any => {
                expect(JSON.parse(req.body)).toStrictEqual({
                    device: {
                        networkAccessIdentifier: "test-device@testcsp.net",
