...
export class DeviceStatusAPI {
    ...
    async subscribe(device: Device, eventType: string, notificationUrl: string, optionalArgs?: SubscribeOptionalArgs): Promise<Subscription> {
        ...
        if (optionalArgs) {
            ...
            if (optionalArgs.subscriptionExpireTime) {
                body.subscriptionExpireTime = optionalArgs.subscriptionExpireTime;
            }

            if (optionalArgs.notificationAuthToken) {
                body.webhook.notificationAuthToken = optionalArgs.notificationAuthToken;
            }
        }
    ...
    async delete(eventSubscriptionId: string) {
        const response = await fetch(`${this.baseUrl}/event-subscriptions/${eventSubscriptionId}`, {
            method: "DELETE",
            headers: this.headers
        })
    }
